...
def test_chunk_pipeline_dummy_request_uses_t2v_dmd_contract(
    ...
    monkeypatch.setattr(
        "vllm_omni.diffusion.diffusion_engine.supports_multimodal_input",
        lambda od_config: (True, True),
    )
    monkeypatch.setattr(
        "vllm_omni.diffusion.diffusion_engine.image_color_format",
        lambda model_class_name: "RGB",
    )
    get_num_frames = Mock(return_value=1)
    monkeypatch.setattr("vllm_omni.diffusion.diffusion_engine.get_dummy_run_num_frames", get_num_frames)

    request = engine._make_dummy_request(
        height=32,
        width=64,
        guidance_scale=5.5,
        num_image_inputs=2,
